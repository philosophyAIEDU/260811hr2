...
var Chat = (function () {
  ...
  function 연결하기() {
    ...
    // 대화상자 안의 버튼들 (목록을 새로 그릴 때마다 생기므로 위임)
    요소('대화상자').addEventListener('click', function (e) {
      var 복사버튼 = e.target.closest('.복사버튼');
      if (복사버튼) { 복사하기(복사버튼); return; }

      var 출처버튼 = e.target.closest('.출처버튼');
      if (출처버튼) 출처원문보기(parseInt(출처버튼.dataset.인덱스, 10));
    });
  }
  ...
  function 빈응답안내문구(중단이유) {
    ...
    if (중단이유 === 'MAX_TOKENS') {
      return 'AI가 답을 다 만들기 전에 멈췄습니다. 질문을 더 짧고 구체적으로 좁혀서 다시 시도해 주세요.';
    }
  ...
  function 말풍선HTML(항목, 인덱스) {
    ...
    // 출처를 누르면 규정 원문을 그대로 띄워, 답변이 맞는지 직접 대조할 수 있다.
    var 출처칸 = 항목.source
      ? '<button type="button" class="말풍선-출처 출처버튼" data-인덱스="' + 인덱스 + '">' +
          '📎 출처: ' + UI.안전한글자(항목.source) +
          '<span class="출처-확인">원문 확인 →</span>' +
        '</button>'
      : '';
  ...
  /* =====================================================================
     6-1. 출처 원문 보기 (교차 검증)
     -------------------------------------------------------------------
     답변 아래 출처를 누르면, 그 근거가 된 규정 원문을 그대로 띄워
     AI가 제대로 답했는지 직접 눈으로 대조할 수 있게 한다.
     ===================================================================== */
  function 출처원문보기(인덱스) {
    var 항목 = 기록[인덱스];
    if (!항목 || !항목.source) return;

    var 출처글 = 항목.source;

    DB.getAllDocuments().then(function (자료들) {
      return DB.getAllChunks().then(function (조각들) {

        // 출처 글에 이름이 등장하는 자료를 찾는다.
        var 해당자료들 = 자료들.filter(function (자료) {
          return 출처글.indexOf(자료.name) !== -1;
        });

        if (해당자료들.length === 0) {
          UI.쪽지('출처로 표시된 자료를 찾지 못했습니다. 자료가 삭제되었을 수 있습니다.', '오류');
          return;
        }

        var 보여줄항목들 = [];

        해당자료들.forEach(function (자료) {
          // 그 자료의 조각 중, 소제목이 출처 글에 등장하는 것을 찾는다.
          // (조각이 길어 나뉜 경우 소제목 뒤에 "(1/2)"가 붙으므로 떼고 비교한다)
          var 맞는조각들 = 조각들.filter(function (조각) {
            if (조각.docId !== 자료.id) return false;
            var 순수소제목 = String(조각.title || '').replace(/\s*\(\d+\/\d+\)\s*$/, '').trim();
            return 순수소제목.length > 0 && 출처글.indexOf(순수소제목) !== -1;
          });

          if (맞는조각들.length > 0) {
            맞는조각들.forEach(function (조각) {
              보여줄항목들.push({
                이름: 자료.name + ' — ' + 조각.title,
                글: 조각.body,
                강조여부: true
            ...
            // 소제목까지는 못 찾았으면 그 자료 전체를 보여준다.
            보여줄항목들.push({ 이름: 자료.name + ' (전체)', 글: 자료.text, 강조여부: true });
          }
        });

        UI.문서창열기({
          제목: '답변의 근거가 된 규정 원문',
          설명: '아래 내용과 답변이 맞는지 직접 비교해 보세요. 노란색 부분이 AI가 근거로 든 조항입니다.',
          항목들: 보여줄항목들,
          주버튼: (해당자료들.length === 1)
            ? {
                글: '이 파일 전체 보기',
                실행: function () {
                  UI.문서창열기({
                    제목: 해당자료들[0].name,
                    설명: UI.숫자표기(해당자료들[0].charCount) + '자 · 파일 전체 내용',
                    항목들: [{ 이름: 해당자료들[0].name, 글: 해당자료들[0].text }]
                  });
                }
              }
            : null
