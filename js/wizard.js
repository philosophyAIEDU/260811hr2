    ...
    지금컨트롤러 = Gemini.질문하기({
      ...
      onDone: function (중단이유) {
        ...
        if (누적글자.trim().length === 0) {
          ...
          var 안내 = (중단이유 === 'SAFETY' || 중단이유 === 'BLOCKED')
            ? 'AI 안전 정책에 걸려 정리하지 못했습니다. 답변 내용을 조금 바꿔 [다시 정리하기]를 눌러 주세요.'
            : (중단이유 === 'MAX_TOKENS')
              ? '한 번에 정리할 내용이 너무 많습니다. 주제를 조금 나누어 다시 시도해 주세요.'
              : 'AI가 정리하지 못했습니다. [다시 정리하기]를 눌러 다시 시도해 주세요.';
          UI.쪽지(안내, '오류');
